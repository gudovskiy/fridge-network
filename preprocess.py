...
class Preprocesser(object):
    ...
    def readFiles(self, instancesFilename):
        ...
        print('Reading saved files...', instancesFilename, config.questionDictFile(), config.answerDictFile(), config.qaDictFile())
    ...
    def writeFiles(self, instances, instancesFilename):
        ...
        if not ('debug' in instancesFilename):
            with open(config.questionDictFile(), "wb") as outFile:
                pickle.dump(self.questionDict, outFile, protocol=3)

            with open(config.answerDictFile(), "wb") as outFile:
                pickle.dump(self.answerDict, outFile, protocol=3)

            with open(config.qaDictFile(), "wb") as outFile:
                pickle.dump(self.qaDict, outFile, protocol=3)
    ...
    # Reads FRIDGR data entries and create a json dictionary.
    def readFRIDGR(self, datasetFilename, instancesFilename, train):
        instances = []
        ...
        if os.path.exists(instancesFilename):
            ...
            if not ('debug' in instancesFilename):
                ...
                print('Create Vocab using ', instancesFilename)
                self.questionDict.createVocab()
                self.answerDict.createVocab()
                self.qaDict.createVocab()
    ...
    def readData(self, datasetFilename, instancesFilename, train):
        # data extraction
        datasetReader = {
            "FRIDGR": self.readFRIDGR,
            "CLEVR": self.readCLEVR,
            "NLVR": self.readNLVR
        }
        print('readData():', datasetFilename, instancesFilename, train)
    ...
    '''
    def readDataset(self, suffix = "", hasTrain = True, hasDebug = False):
        
        if not hasDebug:
            dataset = {"train": None, "evalTrain": None, "val": None, "test": None}
            # no val/test for "real" data
            dataset["val"] = self.readTier("val", train = False)
            dataset["test"] = self.readTier("test", train = False)
            #dataset["val"] = self.readTier(suffix + "val", train = False)
            #dataset["test"] = self.readTier(suffix + "test", train = False)
            if hasTrain:
                dataset["train"] = self.readTier(suffix + "train", train = True)
            ...
            dataset = {"debug": None}
            dataset["debug"] = self.readTier(suffix + "debug", train = False)
    ...
    '''
    def prepareData(self, data, train, filterKey = None, noBucket = False, hasDebug = False):
        ...
        if not hasDebug:
            ...
            filterDebug = {"maxQLength": config.vMaxQ, "maxPLength": config.vMaxP,
                        "onlyChain": config.vOnlyChain, "filterOp": config.vFilterOp}

            filters = {"debug": filterDebug}
        ...
        if (not train) and (not config.retainVal) and (not hasDebug):
            random.shuffle(data)
        if num > 0:
            data = data[:num]
    ...
    def prepareDataset(self, dataset, noBucket = False, hasDebug = False):
        if dataset is None:
            return None
        ...
        for tier in dataset:
            if dataset[tier] is not None:
                dataset[tier]["data"] = self.prepareData(dataset[tier]["instances"], 
                    train = dataset[tier]["train"], filterKey = tier, noBucket = noBucket, hasDebug = hasDebug)
    ...
    def initializeWordEmbeddings(self, wordsDict = None, noPadding = False):
        ...
        print(counter, embeddings.shape)
    ...
    '''
    def preprocessData(self, hasTrain = True, hasDebug = False):
        # Read data into json and symbols' dictionaries
        print(bold("Loading data..."))
        start = time.time()
        mainDataset = self.readDataset(hasTrain = hasTrain, hasDebug = hasDebug)

        extraDataset = None
        if config.extra:
            # compositional Clevr doesn't have training dataset
            extraDataset = self.readDataset(suffix = "real", hasTrain = (not config.extraVal))          
        ...
        mainDataset = self.prepareDataset(mainDataset, hasDebug = hasDebug)
