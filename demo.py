...
def printDatasetResults(trainRes, evalRes, extraEvalRes):
    ...
    #printTierResults("Test", evalRes["test"], "red")
    printTierResults("Debug", evalRes["debug"], "cyan")
...
def writePreds(preprocessor, evalRes, extraEvalRes):
    ...
    preprocessor.writePreds(evalRes["debug"], "debug")
...
def runEvaluation(sess, model, data, epoch, evalTrain = True, evalTest = False, evalDebug = False, getAtt = None):
    if getAtt is None:
        getAtt = config.getAtt
    res = {"evalTrain": None, "val": None, "test": None, "debug": None}
    ...
    if data is not None:
        ...
        if evalDebug:
            res["debug"] = runEpoch(sess, model, data["debug"], train = False, epoch = epoch, getAtt = getAtt)
        
    return res

# Runs demo
def runDemo(sess, model, data, epoch, getAtt = None):
    if getAtt is None:
        getAtt = config.getAtt
    res = {"debug": None}
    
    res["debug"] = runInference(sess, model, data["debug"], idx, train = False, epoch = epoch, getAtt = getAtt)
...
'''# Batches data into a single batch.
def getBatch(data):

    return selectIndices(data, idx)'''
...
'''
Runs an inference on a single image.
...
Args:
    ...
    idx: data index to use for evaluation.
    ...
    getAtt: True to return model attentions.
'''
def runInference(sess, model, data, idx, train, epoch, saver = None, calle = None, 
    alterData = None, getAtt = False):
    ...
    # prepare bucket
    bucket = data["data"]
    dataLen = getLength(bucket)
    assert dataLen == 1, "Only one question per forward pass!"
    batchNum = 0
    #batch = getBatch(bucket, idx = idx)
    batch = bucket
    startTime = time.time()
    # prepare batch 
    batch = trimData(batch)
    print('B:', batchNum, batch)
    # load images batch
    imagesBatch = loadImageBatch(data["images"], batch)
    for i, imageId in enumerate(batch["imageIds"]):
        assert imageId == imagesBatch["imageIds"][i]
    ...
    # run batch
    #print('imagesBatch =', imagesBatch["images"].shape)
    res = model.runBatch(sess, batch, imagesBatch, train, getAtt)
    #print('RES',res)
    #print('RES[RPEDS]',res["preds"])
