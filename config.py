...
def parseArgs():
    ...
    parser.add_argument("--featureType",    default = "cls_resnet101", type = str, help = "features type") #   
    ...
    # sgd optimizer
    parser.add_argument("--batchSize",      default = 64, type = int,       help = "batch size (default=1)")    
...
def configFRIDGR():
    config.dataPath = "{dataBasedir}/FRIDGR_v0.1/data".format(dataBasedir = config.dataBasedir)
    config.datasetFilename = "FRIDGR_{tier}_questions.json"
    config.wordVectorsFile = "./FRIDGR_v0.1/data/glove/glove.6B.{dim}d.txt".format(dim = config.wrdEmbDim) #
    ...
    '''config.typeFilters = [[], ["1_query_size_",
                "1_query_freshness_",
                "2_equal_category_",
                "2_equal_shape_"],
                ["1_query_category_",
