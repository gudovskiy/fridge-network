...
parser.add_argument("--expName",  default = "fridgrExperiment0", type = str)
...
# Dirs
parser.add_argument("--imageBasedir", default = "./FRIDGR_v0.1", type = str, help = "image base directory")
...
imagesDir = "{dir}/{tier}".format(
    dir = args.imageBasedir,
    tier = args.tier)

dataFile = "{dir}/preds/{expName}/{tier}Predictions-{expName}.json".format(
    dir = args.dataBasedir,
    tier = args.tier, 
    expName = args.expName)

inImgName = lambda index: "{dir}/{dataset}_{tier}_{index}.png".format(
    dir = imagesDir,
...
def main():
    ...
    for i in args.instances:        
        ...
        table = np.array(results[i]["attentions"]["question"])[:,:(len(questionList) + 0)]# + 1)]
        #print(iterations, questionList)
